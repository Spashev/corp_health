...
class MainServiceController extends Controller
{
    public function index(Request $request, MainServicesService $service): AnonymousResourceCollection
    {
        ...
        $services = $service->getServices($type);

        return GetMainServicesResource::collection($services);
    }
    ...
    {
        $type = $request->segment(3);
        $serviceObj = $service->getServiceById($id, $type);

        return GetMainServiceResource::make($serviceObj);
    }
